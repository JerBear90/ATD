...
shippingfromstatedisp = shippingfromstate
...
shippingtostatedisp = shippingtostate
...
Else
	If totalprice = "error1" Then
		response.redirect "/index-wp.asp?error=badfrom"
	ElseIf totaltemp = "error2" Then
		response.redirect "/index-wp.asp?error=badto"
	Else
		response.redirect "/index-wp.asp?error=badunknown"
	End If
End If
...
<div class="title1" style="text-align:center;font-size:22px;">Book It Now Securely Online!<br>
...
<div class="stateleft">
    ...
    If len(Session("username")) > 0 and session("emailrefer") = "" Then
    %>
        <img src="//d36b03yirdy1u9.cloudfront.net/images/states/<%=shippingfromstateabbr%>.jpg" />
    <% else %>
        <div style="text-align:center">
            <img src="//d36b03yirdy1u9.cloudfront.net/images/states/<%=shippingfromstateabbr%>.jpg" />
            <div style="margin:10px 0; font-weight:bold;font-size:1.2em;"><%= shippingfromstatedisp %></div>
        </div>
    <% end if %>
    ...
    <% If len(Session("username")) > 0 and session("emailrefer") = "" Then %>
    	...
    	<select name="reviewdiscountprice" id="reviewdiscountprice" onchange="reviewdiscount();" style="width:100%">
            ...
            <option value="AgreedToReview">Agreed to Review ($-25)</option>
    	</select>
    	</form>
    	<br>
...
<div class="stateright">
    ...
    If len(Session("username")) > 0 and session("emailrefer") = "" Then
    %>
        <img src="//d36b03yirdy1u9.cloudfront.net/images/states/<%=shippingtostateabbr%>.jpg" />
    <% else %>
        <div style="text-align:center">
            <img src="//d36b03yirdy1u9.cloudfront.net/images/states/<%=shippingtostateabbr%>.jpg" />
            <br>
            <div style="margin-top:15px; font-weight:bold;font-size:1.2em"><%= shippingtostatedisp %></div>
        </div>
    <% end if %>
...
<input type="hidden" name="strtotalprice" id="totalprice" value="<%= totalprice %>">
...
<tr>
	<td class="bodytext5" valign="top">Shipping From:</td>
	<td class="bodytext5" valign="top"><b><%= shippingfromcity %>, <%= shippingfromstateabbr %>&nbsp;&nbsp;&nbsp;<%=shippingfromzip%></b>
	<% If len(Session("username")) > 0 and session("emailrefer") = "" Then %>
    	<br>
...
<tr>
	<td class="bodytext5" valign="top">Shipping To:</td>
	<td class="bodytext5" valign="top"><b><%= shippingtocity %>, <%= shippingtostateabbr %>&nbsp;&nbsp;&nbsp;<%=shippingtozip%></b>
	<% If len(Session("username")) > 0 and session("emailrefer") = "" Then %>
	<br>
...
<%if howmany <> "onevehicle" then %>
<tr>
	<td class="bodytext5" valign="top">Types of Vehicles:</td>
	<td class="bodytext5" nowrap>
...
<% else %>
<tr>
	<td class="bodytext5">Type of Vehicle:</td>
	<td class="bodytext5" nowrap><b><%= auto_year %> - <%= auto_make %> - <%= auto_model %></b></td>
</tr>
<% end if %>

<tr>
	<td class="bodytext5" nowrap="nowrap">Operating Condition:&nbsp;&nbsp;</td>
	<td class="bodytext5" nowrap="nowrap"><b><%= vehicle_operational %> and Rolls, Brakes, Steers</b></td>
</tr>
<tr>
	<td class="bodytext5">Type of Trailer:</td>
	<td class="bodytext5"><b><%= vehicle_trailer %></b></td>
</tr>
...
<tr>
	<td colspan="2"><br />
	<div class="title2" style="color: #308dff; font-size: 25px;" align="center"><b>Total Price:</b> $<span id="totalprice"><%= totalprice  %></span></div>
	</td>
</tr>
...
<br/>
<% If len(Session("username")) > 0 and session("emailrefer") = "" and (Message_To <> "" or Message_From <> "") Then %>
	<input type="button" id="confirmLink" value="Go On &raquo;" class="submitbutton2"><br />to set-up shipment.
<% Else %>
	<input type="submit" value="Go On &raquo;" class="submitbutton2"><br />to set-up shipment.
<% End If %>
</font></b></div>
-->
...
$(document).ready(function() {
    ...
    $("#confirmLink2").click(function(e) {
		e.preventDefault();
		$('#totalprice').val('<%=expeditedprice%>');
        $('#pricetier').val('2');

        if ($("input[name='auto_price']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price']").val()) + <%=expeditedrate%>
            $("input[name='auto_price']").val(auto_price_new);
        }
        if ($("input[name='auto_price2']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price2']").val()) + <%=expeditedrate%>
            $("input[name='auto_price2']").val(auto_price_new);
        }
        if ($("input[name='auto_price3']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price3']").val()) + <%=expeditedrate%>
            $("input[name='auto_price3']").val(auto_price_new);
        }
        if ($("input[name='auto_price4']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price4']").val()) + <%=expeditedrate%>
            $("input[name='auto_price4']").val(auto_price_new);
        }
        if ($("input[name='auto_price5']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price5']").val()) + <%=expeditedrate%>
            $("input[name='auto_price5']").val(auto_price_new);
        }
        <% If len(Session("username")) > 0 and session("emailrefer") = "" and (Message_To <> "" or Message_From <> "") Then %>
            repcheck();
        <% Else %>
    ...
    $("#confirmLink3").click(function(e) {
		e.preventDefault();
		$('#totalprice').val('<%=rushprice%>');
        $('#pricetier').val('3');

        if ($("input[name='auto_price']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price']").val()) + <%=rushrate%>
            $("input[name='auto_price']").val(auto_price_new);
        }
        if ($("input[name='auto_price2']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price2']").val()) + <%=rushrate%>
            $("input[name='auto_price2']").val(auto_price_new);
        }
        if ($("input[name='auto_price3']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price3']").val()) + <%=rushrate%>
            $("input[name='auto_price3']").val(auto_price_new);
        }
        if ($("input[name='auto_price4']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price4']").val()) + <%=rushrate%>
            $("input[name='auto_price4']").val(auto_price_new);
        }
        if ($("input[name='auto_price5']").val() != '') {
            var auto_price_new = parseInt($("input[name='auto_price5']").val()) + <%=rushrate%>
            $("input[name='auto_price5']").val(auto_price_new);
        }
        <% If len(Session("username")) > 0 and session("emailrefer") = "" and (Message_To <> "" or Message_From <> "") Then %>
            repcheck();
...
<div style="font-weight:bold;border-top: 1px solid #CCC;padding-top:10px;">
Hours of Operation – Weekdays&nbsp;&nbsp;|&nbsp;&nbsp;Eastern: 9:00 am – 8:00 pm&nbsp;&nbsp;|&nbsp;&nbsp;Central: 8:00 am – 7:00 pm&nbsp;&nbsp;|&nbsp;&nbsp;Mountain: 7:00 am – 6:00 pm&nbsp;&nbsp;|&nbsp;&nbsp;Pacific: 6:00 am – 5:00 pm
</div>

<div class="setupship">
    <% 
    balancestandard = totalprice - DEATDeposit
    balanceexpedited = expeditedprice - DEATDeposit
    balancerush = rushprice - DEATDeposit
    %>
    <div class="setupshipcol">
        <div class="pricerow">Standard Price <strong>$<%= totalprice  %></strong></div>
        ...
        Credit Card Deposit $<%=DEATDeposit%><br>
        Balance To Driver At Destination $<%=balancestandard%>
        </div>
    </div>

    <div class="setupshipcol">
        <div class="pricerow">Expedited Price <strong>$<%= expeditedprice  %></strong></div>
        ...
        <div class="priceexplain">
        Credit Card Deposit $<%=DEATDeposit%><br>
        Balance To Driver At Destination $<%=balanceexpedited%>
        </div>
    </div>

    <div class="setupshipcol">
        <div class="pricerow">Rush Price <strong>$<%= rushprice  %></strong></div>
        ...
        <div class="priceexplain">
        Credit Card Deposit $<%=DEATDeposit%><br>
        Balance To Driver At Destination $<%=balancerush%>
        </div>
    </div>
    <div style="clear:both;"></div>
</div>
...
<% If len(Session("username")) > 0 and session("emailrefer") = "" Then %>
	<div style="width:420px;border:2px solid #000;background:#eee;padding:5px;margin: 0 auto; margin-bottom:15px;">
        ...
        <%
        If len(Session("username")) > 0 and session("emailrefer") = "" Then
        	qs = "cd50url=" & cd50 & "&cd100url=" & cd100 & "&sendfrom=" & Server.URLEncode(shippingfromcity) & ", " & Server.URLEncode(shippingfromstate) & "  " & Server.URLEncode(shippingfromzip) & "&sendto=" & Server.URLEncode(shippingtocity) & ", " & Server.URLEncode(shippingtostate) & "  " & Server.URLEncode(shippingtozip) & "&numvehicles=" & Server.URLEncode(numvehicles) & "&vehicle=" & Server.URLEncode(auto_make) & " - " & Server.URLEncode(auto_model) & "&vehicle2=" & Server.URLEncode(auto_make2) & " - " & Server.URLEncode(auto_model2) & "&vehicle3=" & Server.URLEncode(auto_make3) & " - " & Server.URLEncode(auto_model3) & "&vehicle4=" & Server.URLEncode(auto_make4) & " - " & Server.URLEncode(auto_model4) & "&vehicle5=" & Server.URLEncode(auto_make5) & " - " & Server.URLEncode(auto_model5) & "&vehicle6=" & Server.URLEncode(auto_make6) & " - " & Server.URLEncode(auto_model6) & "&vehicle7=" & Server.URLEncode(auto_make7) & " - " & Server.URLEncode(auto_model7) & "&vehicle8=" & Server.URLEncode(auto_make8) & " - " & Server.URLEncode(auto_model8) & "&vehicle9=" & Server.URLEncode(auto_make9) & " - " & Server.URLEncode(auto_model9) & "&vehicle10=" & Server.URLEncode(auto_make10) & " - " & Server.URLEncode(auto_model10) & "&operating=" & Server.URLEncode(vehicle_operational) & Server.URLEncode(" and Rolls, Brakes, Steers") & "&trailer=" & Server.URLEncode(vehicle_trailer) & "&deposit=" & Server.URLEncode(DEATDeposit) & "&price=" & Server.URLEncode(formatnumber(totalprice))

        	%>
        	<a href="priceissue_email.asp?<%= qs %>" rel="shadowbox;width=700;height=200" style="text-decoration:none;">Report Price Issue To Mike</a>
        	<br>
        <% end if %>
    ...
    <div style="border-top: 1px solid #ccc"></div><br><br>
<%end If %>
...
<p>Place a nominal deposit of <strong>$<%=strDeposit%></strong> to set up your shipment, which is included in your quote.</p>
...
<!--
<p>
    <div style="margin:15px 0 20px;" class="trustpilot-widget" data-locale="en-US" data-template-id="53aa8912dec7e10d38f59f36" data-businessunit-id="5195d470000064000530e14c" data-style-height="130" data-style-width="930px"></div>
</p>
-->
...
<div class="wpb_row vc_row-fluid quoteblock" style="margin-top:30px !important; position: relative;left: -60px;width: 1190px;">
	...
	<div class="wpb_text_column wpb_content_element ">
		<div class="wpb_wrapper">
			<div id="promobar">
<div class="promo-box" style="margin-right:7px;">
<p><img alt="car carrier" src="//d36b03yirdy1u9.cloudfront.net/images-v3/auto_transport_promo_v2-1.jpg" width="286" height="195"></p>
<div class="promo-text">This is the place to perform auto transport quotes to ship your car reliably.</div>
</div>
<div class="promo-box" style="margin-right:7px;">
<p><img alt="Auto Transport To Your State" src="http://www.autotransportdirect.com/services/stateimage/index.php" width="286" height="195"></p>
<div class="promo-text">We were the first to have an online state to state car shipping quotes calculator.</div>
</div>
<div class="promo-box" style="margin-right:7px;">
<p><img alt="Auto Transport Representative" src="//d36b03yirdy1u9.cloudfront.net/images-v3/auto_transport_promo_v2-3.jpg" width="286" height="195"></p>
...
<p><img alt="Auto Transport Family" src="//d36b03yirdy1u9.cloudfront.net/images-v3/auto_transport_promo_v2-4.jpg" width="286" height="195"></p>
